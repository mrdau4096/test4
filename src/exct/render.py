...
def PROCESS_OBJECT(OBJECT_DATA, PLAYER, COPIED_VERTS, COPIED_INDICES):
	...
	if OBJECT_TYPE in [SPRITE_STATIC, ITEM, ENEMY]:
		...
		NORMALS = ((COORDINATES[1] - COORDINATES[0]).CROSS(COORDINATES[2] - COORDINATES[0]), (COORDINATES[1] - COORDINATES[3]).CROSS(COORDINATES[2] - COORDINATES[3]))
		if OBJECT_TYPE in [ENEMY,]:
			...
			match ANGLE:
				case N if N > 100 or N <= -100:
					TEXTURE = OBJECT_DATA.TEXTURE_INFO[0]
				
				case N if N <= 100 and N > 60:
					TEXTURE = OBJECT_DATA.TEXTURE_INFO[1]

				case N if N <= 60 and N > 20:
					TEXTURE = OBJECT_DATA.TEXTURE_INFO[2]

				case N if N <= 20 and N > -20:
					TEXTURE = OBJECT_DATA.TEXTURE_INFO[3]

				case N if N <= -20 and N > -60:
					TEXTURE = OBJECT_DATA.TEXTURE_INFO[4]
				
				case N if N <= -60 and N > -100:
					TEXTURE = OBJECT_DATA.TEXTURE_INFO[5]

		else:
			TEXTURE = OBJECT_DATA.TEXTURE_INFO[0]

		COPIED_VERTS, COPIED_INDICES = OBJECT_VAO_MANAGER(OBJECT_TYPE, NORMALS, COORDINATES, TEXTURE, [COPIED_VERTS, COPIED_INDICES])
...
def OBJECT_VAO_MANAGER(CLASS_TYPE, NORMALS, POINTS, TEXTURE_DATA, VAO_DATA):
	...
	if CLASS_TYPE == CUBE_STATIC: #Cubes
		FACE_ORDER = [
			(0, 1, 3, 2),	# -Y | Bottom
			(0, 2, 6, 4),	# -X | Left
			(5, 7, 3, 1),	# +X | Right
		...
		#print(f"{hex(maths.floor((1-V)*16))[2:].upper()}{hex(maths.floor(U*16))[2:].upper()}")
