...
def PROCESS_OBJECT(OBJECT_DATA, PLAYER, COPIED_VAO_DATA):
	...
	if OBJECT_TYPE in [SPRITE_STATIC, ITEM, ENEMY]:
		...
		COPIED_VAO_DATA = OBJECT_VAO_MANAGER(OBJECT_DATA, COPIED_VAO_DATA, TEXTURES=TEXTURES, POINTS=COORDINATES)

	elif OBJECT_TYPE in [CUBE_PHYSICS,]:
		NORMALS = utils.FIND_CUBOID_NORMALS(OBJECT_DATA.POINTS)
		COPIED_VAO_DATA = OBJECT_VAO_MANAGER(OBJECT_DATA, COPIED_VAO_DATA)
...
def OBJECT_VAO_MANAGER(OBJECT, ENV_VAO_DATA, TEXTURES=None, POINTS=None):
	...
	if CLASS_TYPE in (CUBE_STATIC, CUBE_PHYSICS, CUBE_PATH):  # Cubes
		...
		for FACE_INDEX, TEXTURE_DATA in enumerate(CUBE_TEXTURE_DATA):
			INDEX_OFFSET = len(ENV_VAO_VERTICES) // 9  # Assuming each vertex has 9 components
			NORMAL = OBJECT.NORMALS[FACE_INDEX]
			FACE_INDICES = FACE_ORDER[FACE_INDEX]
			INDICES_OFFSET = [INDEX_OFFSET + I for I in range(4)]

			NEW_INDICES[FACE_INDEX * 6:FACE_INDEX * 6 + 6] = NP.array([
			...
			for I, INDEX in enumerate(FACE_INDICES):
				...
				TEX_COORD = TEXTURE_DATA[1][I]
				U, V = TEX_COORD.X, TEX_COORD.Y
				ID = texture_load.SHEET_INDEX_LIST.index(TEXTURE_DATA[0])
				NEW_VERTICES.append([
					X, Y, Z,
					U, V,
			...
			ENV_VAO_VERTICES.extend(NEW_VERTICES)
			ENV_VAO_INDICES = NP.concatenate((ENV_VAO_INDICES, NEW_INDICES)).flatten()
		...
		VERTICES, INDICES = BUFFER_DATA[TEXTURES[0]]
		INDEX_OFFSET = len(VERTICES) // 9  # Assuming each vertex has 9 components
		...
		for I, INDEX in enumerate(FACE_ORDER):
			...
			X, Y, Z = OBJECT.POINTS[INDEX].X, OBJECT.POINTS[INDEX].Y, OBJECT.POINTS[INDEX].Z
			U, V = TEX_COORD.X, TEX_COORD.Y
			ID = texture_load.SHEET_INDEX_LIST.index(TEXTURES[0][1])
			NEW_VERTICES.append([
				X, Y, Z,
				U, V,
				NORMAL.X, NORMAL.Y, NORMAL.Z,
...
def FBO_QUAD_INIT(RENDER_RES):
	...
	FBO_SCENE, TCB_SCENE, _, _, _ = CREATE_FBO(RENDER_RES)

	return VAO_QUAD, VAO_UI, FBO_SCENE, TCB_SCENE
...
def BUFFERS_INIT(VERTICES=None, INDICES=None):
	...
	VERTEX_SIZE_BYTES = 3 * 4 + 2 * 4 + 3 * 4 + 1 * 4

	#Position XYZ
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, VERTEX_SIZE_BYTES, ctypes.c_void_p(0))
	glEnableVertexAttribArray(0)

	#UV Coordinate XY
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, VERTEX_SIZE_BYTES, ctypes.c_void_p(12))
	glEnableVertexAttribArray(1)

	#Normal Dir XYZ
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, VERTEX_SIZE_BYTES, ctypes.c_void_p(20))
	glEnableVertexAttribArray(2)

	#Sheet ID X
	glVertexAttribIPointer(3, 1, GL_INT, VERTEX_SIZE_BYTES, ctypes.c_void_p(32))
...
def RENDER_DEPTH_MAP(VAO_SHADOW, SHADOW_SHADER, PROJECTION, VIEW, FBO_SHADOW, SHADOWMAP_RESOLUTION, ENV_VAO_INDICES):
...
def CREATE_LIGHT_MAPS(LIGHT, VAO_DATA, SHADOW_SHADER, SHADOWMAP_RESOLUTION):
	VAO_VERTICES, VAO_INDICES = VAO_DATA
	...
	VAO_SHADOW, VBO_SHADOW, EBO_SHADOW = BUFFERS_INIT(VERTICES=NP.array(VAO_VERTICES), INDICES=NP.array(VAO_INDICES).flatten())
	FBO_SHADOW, _, DTB_SHADOW, _, _ = CREATE_FBO(SHADOWMAP_RESOLUTION, DEPTH=True, NORMALS=True)
	...
	RENDER_DEPTH_MAP(VAO_SHADOW, SHADOW_SHADER, LIGHT_PROJECTION_MATRIX, LIGHT_VIEW_MATRIX, FBO_SHADOW, SHADOWMAP_RESOLUTION, VAO_INDICES)
