...
def UPDATE_PHYSICS(PHYS_DATA, FPS, KEY_STATES, FLAG_STATES):
	...
	try:
					...
					for BODY_ID, BODY in KINETICs_LIST.items():
						if BODY_ID != PHYS_OBJECT_ID: #Stops self collisions.
							...
							if BOUNDING_BOX_COLLISION(PHYS_OBJECT.BOUNDING_BOX, BODY.BOUNDING_BOX): #Axis-aligned bounding-box collision check for computational efficiency
								...
								if COLLISION_DATA[0]:
									if OBJECT_TYPE == PROJECTILE and BODY_TYPE not in (PROJECTILE, EXPLOSION,):
										if BODY.ID != PHYS_OBJECT.OWNER:
											if PHYS_OBJECT.CREATE_EXPLOSION:
												...
												HURT_OBJECTS[PHYS_OBJECT_ID] = (BODY_ID, BODY.DAMAGE_STRENGTH)

											REMOVED_OBJECTS.append(PHYS_OBJECT_ID)
											continue


									elif BODY_TYPE == PROJECTILE and OBJECT_TYPE not in (PROJECTILE, EXPLOSION,):
										if PHYS_OBJECT.ID != BODY.OWNER:
											if BODY.CREATE_EXPLOSION:
												...
												HURT_OBJECTS[BODY_ID] = (PHYS_OBJECT_ID, PHYS_OBJECT.DAMAGE_STRENGTH)
										...
										if PHYS_OBJECT.EXPLODED: continue
										DIRECTION = BODY.POSITION - PHYS_OBJECT.POSITION
										DIMS = BODY.DIMENTIONS_3D if BODY_TYPE in (ITEM,) else BODY.DIMENTIONS
										DISTANCE_REDUCTION = 1.0 - (abs(DIRECTION) / ((DIMS.X + DIMS.X)*1.25))
										HURT_OBJECTS[PHYS_OBJECT_ID] = (BODY_ID, PHYS_OBJECT.DAMAGE_STRENGTH * DISTANCE_REDUCTION)

										BODY.POSITION.Y += 0.05
										BODY.LATERAL_VELOCITY += DIRECTION.NORMALISE() * PHYS_OBJECT.PUSH_FORCE * DISTANCE_REDUCTION / BODY.MASS


									elif BODY_TYPE == EXPLOSION:
										if BODY.EXPLODED: continue
										DIRECTION = PHYS_OBJECT.POSITION - BODY.POSITION
										DIMS = PHYS_OBJECT.DIMENTIONS_3D if OBJECT_TYPE in (ITEM,) else PHYS_OBJECT.DIMENTIONS
										DISTANCE_REDUCTION = CLAMP(1.0 - (abs(DIRECTION) / ((DIMS.X + DIMS.X)*1.25)), 0.0, 1.0)
										HURT_OBJECTS[BODY_ID] = (PHYS_OBJECT_ID, BODY.DAMAGE_STRENGTH * DISTANCE_REDUCTION)

										PHYS_OBJECT.POSITION.Y += 0.05
										PHYS_OBJECT.LATERAL_VELOCITY += DIRECTION.NORMALISE() * BODY.PUSH_FORCE * DISTANCE_REDUCTION / PHYS_OBJECT.MASS
		...
		for DANGER_ID, OBJECT_DATA in HURT_OBJECTS.items():
			OBJECT_ID, DAMAGE = OBJECT_DATA
			if OBJECT_ID in KINETICs_LIST and DANGER_ID in KINETICs_LIST:
				OBJECT, DANGER = KINETICs_LIST[OBJECT_ID], KINETICs_LIST[DANGER_ID]
				if type(OBJECT) in (PLAYER, ENEMY,):
					(KINETICs_LIST, STATICs_LIST) = OBJECT.HURT(DAMAGE, (KINETICs_LIST, STATICs_LIST), scene.CURRENT_ID)
...
def COLLISION_CHECK(PHYS_BODY, STATIC):
	...
	if STATIC_TYPE in (CUBE_STATIC, CUBE_PHYSICS, CUBE_PATH, TRIGGER, PROJECTILE, ITEM, EXPLOSION,): #Cube-like objects
